...
class Base(nn.Module):
    def __init__(self, in_channel, ngf=16):
        super(Base, self).__init__()
        self.branch1 = nn.Sequential(
            nn.Conv2d(in_channel, ngf, kernel_size=1, stride=1, padding=0),
            nn.Conv2d(ngf, ngf * 2, kernel_size=1, stride=1, padding=0),
            nn.Conv2d(ngf * 2, ngf * 4, kernel_size=1, stride=1, padding=0),
        ...
        self.branch3 = nn.Sequential(
            ...
            nn.Conv2d(ngf, ngf, kernel_size=3, stride=1, padding=3, dilation=3),
            nn.BatchNorm2d(ngf),
            nn.ReLU()
        )
        self.branch4 = nn.Sequential(
            ...
            nn.Conv2d(ngf, ngf, kernel_size=3, stride=1, padding=5, dilation=5),
            nn.BatchNorm2d(ngf),
            nn.ReLU()
        )
        self.branch5 = nn.Sequential(
            nn.Conv2d(in_channel, ngf, kernel_size=1, stride=1, padding=0),
            nn.Conv2d(ngf, ngf, kernel_size=7, stride=1, padding=3),
            nn.Conv2d(ngf, ngf, kernel_size=3, stride=1, padding=7, dilation=7),
            nn.BatchNorm2d(ngf),
            nn.ReLU()
        )
        self.layer1 = nn.Conv2d(ngf * 4, ngf * 4, kernel_size=1, stride=1, padding=0)
...
class MarExtract(nn.Module):
    def __init__(self, c_in=64):
        super(MarExtract, self).__init__()
        self.layer = nn.Conv2d(in_channels=256, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.sigmoid = nn.Sigmoid()
        self.relu = nn.ReLU()
        self.Base = Base(c_in, 64)
        self.corr = CorrelationLayer(32, feat_channel=64)
    ...
    def forward(self, fe, guide_fe):
        ...
        #概率生成
        feature = self.Base(guide_fe)
        glo = self.corr(feature)
...
class Edge_en(nn.Module):
    def __init__(self):
        super(Edge_en, self).__init__()
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.margin = MarExtract()
        self.edge_en = Edge_en()
    ...
    def forward(self, ia):
        ...
        fe_edg_1 = self.margin(ia, img1_fe_1)
        fe_1 = self.edge_en(fe_edg_1, img1_fe) + img1_fe
        return fe_1
...
class Decoder(nn.Module):
    ...
    def forward(self, fe_max, mask_unconsist):
        out = self.ImgRebuilt(fe_max, mask_unconsist)
        return out
