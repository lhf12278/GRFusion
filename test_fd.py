import setproctitle
setproctitle.setproctitle('wangdan')
...
fd = U_Net1(3, 2)
fd_path = "./checkpoints/fd_89.pth"
# fd_path = "./checkpoints_fd/kernel_37.pth"
    ...
    if img_num == 2:
        with torch.no_grad():
            for num in range(1, couples + 1):
                ...
                # img1_mask_bi = Getmargin(img1_mask_bi)
                # img2_mask_bi = Getmargin(img2_mask_bi)
                # img1_mask_bi = fill(img1_mask_bi)
                # img2_mask_bi = fill(img2_mask_bi)
                ...
                # unconsis_bi.save(save_path + '/MFI-WHU_{}{}_unconsist.jpg'.format(num // 10, num % 10))
                ...
                # unconsis_bi.save(save_path + '/Lytro_{}{}_unconsist.jpg'.format(num // 10, num % 10))
        ...
        with torch.no_grad():
            # path1 = './mask/lytro-01-A.jpg'
            # path2 = './mask/lytro-01-B.jpg'        # for the "Lytro" dataset
            # path3 = './mask/lytro-01-C.jpg'         # for the "Lytro" dataset
            # img1 = Image.open(path1).convert('RGB')
            # img2 = Image.open(path2).convert('RGB')
            ...
            #
            # img1_mask_bi = out1.mul_(255).add_(0.5).clamp_(0, 255).squeeze(0).permute(1, 2, 0).cpu()
            # print(img1_mask_bi.shape)
            # img2_mask_bi = out2.mul_(255).add_(0.5).clamp_(0, 255).squeeze(0).permute(1, 2, 0).cpu()
            # img3_mask_bi = out3.mul_(255).add_(0.5).clamp_(0, 255).squeeze(0).permute(1, 2, 0).cpu()
            # img_un = unconsis.mul_(255).add_(0.5).clamp_(0, 255).squeeze(0).permute(1, 2, 0).cpu()
            #
            # img1_mask_bi = Image.fromarray(np.uint8(img1_mask_bi))
            # img2_mask_bi = Image.fromarray(np.uint8(img2_mask_bi))
            # img3_mask_bi = Image.fromarray(np.uint8(img3_mask_bi))
            # img_un = Image.fromarray(np.uint8(img_un))
            # img1_mask_bi.save(save_path + '/MFI-WHU_001_A.jpg')
            # img2_mask_bi.save(save_path + '/MFI-WHU_001_B.jpg')
            # img3_mask_bi.save(save_path + '/MFI-WHU_001_C.jpg')
            # img_un.save(save_path + '/MFI-WHU_001_un.jpg')
            #
            # toc = time.time()
            # print('over')
...
if __name__ == '__main__':
    # fusion_color('../triple_sets', 'jpg', '/home/l/data_1/wd/code/Mfif/Canny3/test_fd', 9, 3)
    # fusion_color('./mask', 'jpg', '/home/l/data_1/wd/code/Mfif/Canny3/test_fd', 5, 3)
    fusion_color('../Mfif/Lytro', 'jpg', './test_fusion', 20, 2)     # fuse the "Lytro" dataset
    # fusion_color('../MFI-WHU', 'jpg', './test_fusion', 30, 2)
    # fusion_color('../triple_sets/four', 'jpg', './test_fd', 1, 4)  # fuse the "Lytro" dataset
    # fusion_color('../triple_sets/six', 'jpg', './test_fd', 1, 6)  # fuse the "Lytro" dataset